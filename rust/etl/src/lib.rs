...
//! * 10 points: "Q", "Z",
//!
//! The shiny new Scrabble system instead stores the score per letter, which makes
//! it much faster and easier to calculate the score for a word. It also stores the
//! letters in lower-case regardless of the case of the input letters:
//!
//! * "a" is worth 1 point.
//! * "b" is worth 3 points.
//! * "c" is worth 3 points.
...
/// Transpose the data structure BTreeMap, and make the letters lower case.
pub fn transform(h: &BTreeMap<i32, Vec<char>>) -> BTreeMap<char, i32> {
    let mut btree = BTreeMap::new();
    for (k, vs) in h {
        for v in vs {
            if let Some(ch) = v.to_lowercase().next() {
                btree.insert(ch, *k);
            }
        }
    }
